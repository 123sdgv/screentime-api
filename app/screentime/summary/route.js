...
export async function GET(request) {
  ...
  const openApps = [];
  const appNames = await redis.smembers("screentime:apps");

  for (const app of appNames || []) {
    const raw = await redis.get(`screentime:state:${app}`);
    const state = typeof raw === "string" ? JSON.parse(raw) : raw;

    if (state?.open && state?.startedAt) {
      openApps.push({
        app,
        minutes: minutes(now - Number(state.startedAt))
      });
    }
  }
